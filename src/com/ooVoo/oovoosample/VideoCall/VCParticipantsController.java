	...
	public int getVisibleSurfacesCount() {
		...
		try {
			...
			for (int i = 0; i < numViews; i++) {
				View v = getChildAt(i);
				if (v instanceof PreviewSurfaceHolder && v.getVisibility() == View.VISIBLE) {
					countActiveSurfaces++;
				}
				else if (v instanceof ParticipantVideoSurface && v.getVisibility() == View.VISIBLE) {
					countActiveSurfaces++;
				}
			}
		} catch (Exception ex) {
		...
		try {
			...
			if (mCurrentUIMode == FULL_MODE  && mActiveSurfacesCount <= 1) {
				for (int i = 0; i < mChildCount; i++) {
					...
					if(v instanceof PreviewSurfaceHolder && v.getVisibility() == View.VISIBLE){
						((PreviewSurfaceHolder) v).updatePrivewParams(0, 0, mWidth, viewHeight);
						break;
					}
				}			
			} else {
				...
				for (int i = 0; i < mChildCount; i++) {
					...
					if(v instanceof PreviewSurfaceHolder && v.getVisibility() == View.VISIBLE){
						((PreviewSurfaceHolder) v).updatePrivewParams(xPos, yPos, videoWidth, videoHeight);
						pos++;
					}
					else if (v instanceof ParticipantVideoSurface && v.getVisibility() == View.VISIBLE) {
